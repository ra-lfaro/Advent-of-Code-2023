...
Your puzzle answer was 278755257.
*/
...
const parseAlmanac = (almanac = ALMANAC): [string[], ParsedMapping] => {
  let seeds: string[] = [];
...
const findLowestLocation = (almanac = ALMANAC) => {
  const [seeds, mappings] = parseAlmanac(almanac);

  let lowestPos: number | undefined;

  for (let seed of seeds) {
    let locPos = convertToLocation(+seed, mappings);
